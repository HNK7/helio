...
def signup_list(request, e_id):
    ...
    half = teams.count() / 2
    context['teams'] = teams
    context['teams_1'] = teams[0:half]
    context['teams_2'] = teams[half:]
...
def game_result(request, rfid=None):
    ...
    if rfid:
        sql = """select to_char(a.ctime, 'mm-dd hh:mi:ss AM' ) as ended_at, gameid, b.name, ppdmpr, teamtype, sameteam,
                      ...
                      where shopid=425 and a.rfid=%s  order by gametype, gameid, sameteam""" % (rfid)
    else:
        sql = """select to_char(a.ctime, 'mm-dd hh:mi:ss AM' ) as ended_at, gameid, b.name, ppdmpr, teamtype, sameteam,
                      iswin, a.rfid
                      from v_gamedata3 a join userinfo b on a.rfid=b.rfid where shopid=425 order by gameid, sameteam"""
...
where gc > 1 and b.rfid > 1  order by gametype, diff desc"""
    ...
    # for row in rows:
    #     try:
    #         Card.objects.get(rfid=row[0]).player.full_name
    #     except ObjectDoesNotExist:
    #         # player_name = row[1]
    #         pass
    # raise('debug')
